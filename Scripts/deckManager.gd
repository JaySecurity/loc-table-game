...
func _ready() -> void:
  ...
  for i in range(27):
    ...
    new_card.data = deck.pop_back()
    new_card.global_position = Vector2(xpos + (210 * mult) , ypos)
    mult += 1
...
func load_cards_from_csv() -> void:
  ...
  if file:
    ...
    for line in lines:
      ...
      var image_name = fields[0].strip_escapes().trim_prefix('"').trim_suffix('"')
      image_name = image_name.replace(" ","").to_lower()
      ...
      card_data.highlight = fields[8].strip_escapes().trim_prefix('"').trim_suffix('"')
      card_data.image = load("res://Assets/Cards/"+image_name+".png")
